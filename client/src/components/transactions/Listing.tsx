...
const Listing = ({ transactions }) => {
  ...
  const [filteredLogs, setFilteredLogs] = useState(transactions)
    ...
    <div className='w-full'>
      ...
      <section className='my-6 relative'>
        <p className='text-center text-xs text-gray-500 dark:text-gray-300'>
          Result: {filteredLogs?.length} transactions
        </p>
      ...
      {filteredLogs?.length > 0 && (
        <section>
          <table className='w-full table-fixed text-center'>
            ...
            <tbody>
              {filteredLogs?.slice((currentPage - 1) * logsPerPage, currentPage * logsPerPage).map(transaction => (
                <tr
                  key={transaction.id}
